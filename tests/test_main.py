...
class TestResolveInput:
    ...
    def test_image_file(self, tmp_path):
        ...
        with (
            ...
            patch("src.main.extract_text", return_value="OCR text") as mock_ocr,
        ):
    ...
    def test_image_empty_ocr_exits(self, tmp_path):
        ...
        with (
            patch("src.main.is_image_file", return_value=True),
            patch("src.main.extract_text", return_value=""),
        ):
            with pytest.raises((SystemExit, typer.Exit)):
...
class TestConfirmAndOutput:
    ...
    def test_user_edits_then_saves(self, tmp_path, monkeypatch):
        ...
        with (
            patch("src.main.Prompt.ask", return_value="e"),
...
class TestEditEvent:
    ...
    def test_edit_title(self):
        with (
            patch("src.main.Prompt.ask", side_effect=["title", "New Title"]),
            patch("src.main.typer.confirm", return_value=False),
        ):
    ...
    def test_edit_location_to_empty(self):
        with (
            patch("src.main.Prompt.ask", side_effect=["location", ""]),
            patch("src.main.typer.confirm", return_value=False),
        ):
            result = edit_event(self._event())
    ...
    def test_edit_start_time(self):
        with (
            patch("src.main.Prompt.ask", side_effect=["start_time", "2026-05-01T14:00:00"]),
            patch("src.main.typer.confirm", return_value=False),
        ):
            result = edit_event(self._event())
    ...
    def test_edit_reminder_minutes(self):
        with (
            patch("src.main.Prompt.ask", side_effect=["reminder_minutes", "30"]),
            patch("src.main.typer.confirm", return_value=False),
        ):
            result = edit_event(self._event())
    ...
    def test_edit_reminder_clear(self):
        ...
        with (
            patch("src.main.Prompt.ask", side_effect=["reminder_minutes", ""]),
            patch("src.main.typer.confirm", return_value=False),
        ):
    ...
    def test_edit_multiple_fields(self):
        with (
            ...
            patch("src.main.typer.confirm", side_effect=[True, False]),
        ):
            result = edit_event(self._event())
